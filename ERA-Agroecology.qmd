...
pacman::p_load(
arrow,s3fs,jsonlite,data.table,ggplot2,rnaturalearth,rnaturalearthdata,sf,viridis,treemapify,tidyr,dplyr,knitr,kableExtra
)
...
site_locs <- acdc_dat[!is.na(Latitude) & !is.na(Longitude), .(Latitude, Longitude, Buffer,Code)]
site_locs <- unique(site_locs)

# 2. Load Africa country shapes as sf object
lac <- ne_countries(country = lac_countries, scale = "medium", returnclass = "sf")

# 3. Plot 
ggplot() +
  geom_sf(data = lac, fill = "grey95", color = "grey70", linewidth = 0.3) +
  geom_hex(
    data = site_locs,
