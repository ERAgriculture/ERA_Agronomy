...
# Create formatted strings per row first
...
ERA_data <- ERA_Compiled %>%
  ...
  # ---- Filter summarised data based on species of interest ----
  filter(str_detect(`Species`, str_c(products_of_interest, collapse = "|")))


# ---- Step 2: Construct variety descriptions from the metadata (ONLY after 2020) ----
variety_info <- var %>%
...
# Helper function: Convert a character vector to a natural language list
...
# Generate base practices sentence with rainfed detection
base_practices <- ERA_Compiled %>%
  ...
  mutate(
    `Cropping systems` = case_when(
      ...
      TRUE ~ ""  # empty if no practices and not rainfed
    )
  )

# ---- Step 3: Join variety info into ERA_data ----
