...
- Checking that the coordinates correspond to **agriculturally plausible areas** (e.g., not in oceans, mountains, or cities). 
- Ensuring consistency with the **site description in the publication**. 
- **Correcting erroneous coordinates** (e.g., swapped lat/lon, misplaced decimals). 
- **Geolocating sites** where no coordinates were given, based on textual descriptions. 
- Assigning an appropriate **uncertainty radius** depending on location specificity (e.g., “county” vs “village” vs “research station”). 
